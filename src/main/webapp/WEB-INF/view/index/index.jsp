<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<script src="/resources/fim/js/index.js" type="text/babel"></script>
<div id="container" class="container container__main">            
    <div class="grid-layout">
        <div class="grid-column grid-column9">
            <div class="section section--border section-link">
                <div class="component-area component-area-quicklink">
                    <em class="section-title">
                        Quick Menu (공통)
                        <!-- <button type="button" class="btn btn--open">
                            <span class="sp icon-arrow--round"><span class="blind">펼쳐서 모두 보기</span></span>
                        </button> -->
                    </em>
                    <div class="group-item-info">
                        <ul class="component-group link-box">
                            <li class="component-box">
                                <a href="expense_management/approval/approval_form.html">
                                    <img src="/resources/fim/img/icon_quick_link_1.png" alt="">
                                    <span>기안서 작성</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="expense_management/approval/approval_report.html">
                                    <img src="/resources/fim/img/icon_quick_link_2.png" alt="">
                                    <span>기안서(보고)</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="expense_management/approval/expense.html">
                                    <img src="/resources/fim/img/mon_4_L2.png" alt="" style="width: 74px;margin-top: -9px">
                                    <span style="margin-top:-11px">지출결의서 작성</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="company_card/account/account_info.html">
                                    <img src="/resources/fim/img/mon_2_L2.png" alt="" style="width: 76px;margin-top: -9px">
                                    <span style="margin-top:-14px">법인카드 정산/신청</span>
                                </a>
                            </li>
                        </ul>
                        <ul class="component-group link-box">
                            <li class="component-box">
                                <a href="business_management/work/user_ratio.html">
                                    <img src="/resources/fim/img/icon_quick_link_5.png" alt="">
                                    <span>업무 투입률 입력</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="business_management/budget/plan_result.html">
                                    <img src="/resources/fim/img/icon_quick_link_6.png" alt="">
                                    <span>예산 계획 대비<br>실적조회</span>
                                </a>
                            </li>
                        </ul>
                        <ul class="component-group link-box">
                            <li class="component-box">
                                <a href="http://mdi.smilegate.net/mdi/cust/custInsertPortal.rb?menuId=1622" target="_blank">
                                    <img src="/resources/fim/img/icon_quick_link_7.png" alt="">
                                    <span>거래처 조회</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://trip.smilegate.net" target="_blank">
                                    <img src="/resources/fim/img/icon_quick_link_8.png" alt="">
                                    <span>출장 (TRIP)</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="company_card/account/cd_apply_form.html">
                                    <img src="/resources/fim/img/mon_2_L2.png" alt="" style="width: 76px;margin-top: -9px">
                                    <span style="margin-top:-14px">공용카드 신청</span>
                                </a>
                            </li>
                        </ul>
                        <ul class="component-group link-box">
                            <li class="component-box">
                                <a href="expense_management/workshop.html">
                                    <img src="/resources/fim/img/icon_quick_link_10.png" alt="">
                                    <span>워크샵 신청/정산</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="#">
                                    <img src="/resources/fim/img/icon_quick_link_11.png" alt="">
                                    <span>예산 변경</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/" target="_blank">
                                    <img src="/resources/fim/img/icon_quick_link_12.png" alt="">
                                    <span>전자결재 (EKP)</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="component-area component-area-guidelink">
                    <em class="section-title">회사생활 가이드 (재무/자금)</em>
                    <div class="group-item-info">
                        <ul class="component-group link-box clearfix">
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_1_L2.png" alt="">
                                    <span>부서운영비 규정</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_2_L2.png" alt="">
                                    <span>공용카드 신청</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_3_L2.png" alt="">
                                    <span>출장 일비 규정</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_4_L2.png" alt="">
                                    <span>지출결의서</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_5_L2.png" alt="">
                                    <span>계약/발주/비용<br>기안작성</span>
                                </a>
                            </li>
                            <li class="component-box">
                                <a href="http://ekp.smilegate.net/eNovator/Portal/CompanyLifeGuide/Default.aspx">
                                    <img src="/resources/fim/img/mon_2_L2.png" alt="">
                                    <span>법인카드</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>                    
            <div class="section section--border section--no-padding section-bbs">
                <div class="tab-area tab-area--top">
                    <ul class="lst-tab">
                        <li class="lst-tab__item" v-bind:class="{'lst-tab__item--active' : noti}" @click="selectTab(1)">
                            <a href="#" class="lst-tab__menu">
                                <span class="lst-tab__menu-txt">공지사항</span>
                            </a>
                        </li>
                        <li class="lst-tab__item" v-bind:class="{'lst-tab__item--active' : exchange}" @click="selectTab(2)">
                            <a href="#" class="lst-tab__menu">
                                <span class="lst-tab__menu-txt">환율 정보</span>
                            </a>
                        </li>
                        <li class="lst-tab__item"  v-bind:class="{'lst-tab__item--active' : taxSchedule}" @click="selectTab(3)">
                            <a href="#" class="lst-tab__menu">
                                <span class="lst-tab__menu-txt">월별 세무 일정</span>
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="tab-cnt">
                    <div class="component-area component-area-info" v-bind:class="{'tab-cnt--active' : noti}">
                        <div class="component-group component-group__table">
                            <div class="table">
                                <table>
                                    <caption><span class="blind">공지사항</span></caption>
                                    <colgroup>
                                        <col>
                                        <col style="width: 91px">
                                    </colgroup>
                                    <tbody>
                                        <tr>
                                            <td class="table__td">
                                                <a href="#" class="table__title">
                                                    <span class="table__txt">[필독] SG재무회계실 전체 공지 안내</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">2020/01/21</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="#" class="table__title">
                                                    <span class="table__txt">제무회계 내부 공지입니다.</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">2020/01/02</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="#" class="table__title">
                                                    <span class="table__txt">제무회계 공지 테스트</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">2019/08/10</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="#" class="table__title">
                                                    <span class="table__txt">제무회계 공지 테스트</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">2019/08/10</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="#" class="table__title">
                                                    <span class="table__txt">제무회계 공지 테스트</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">2019/08/10</span>
                                            </td>
                                        </tr>                                    
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div class="component-box component-box--align-right">
                            <a href="portal_management/info.html" class="btn btn--link"><span class="btn__txt">+ 더보기</span></a>
                        </div>
                    </div>
                    <div class="component-area component-area-exchange" v-bind:class="{'tab-cnt--active' : exchange}">
                        <span class="component-area__dsc-txt">{{taxDate}} 기준</span>
                        <div class="component-group component-group__table">
                            <div class="table">
                                <table>
                                    <caption><span class="blind">환율 정보</span></caption>
                                    <colgroup>
                                        <col>
                                        <col>                                        
                                    </colgroup>
                                    <thead>
                                        <tr>
                                            <th scope="col" class="table__th">통화명</th>
                                            <th scope="col" class="table__th table__txt--align-right">매매기준율</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="list in exchangeList">
                                            <td class="table__td">
                                                <span class="table__txt table__txt--blue-dark">{{list.KorCurrName}}</span>
                                            </td>
                                            <td class="table__td table__title">
                                                <span class="table__txt table__txt--align-right">{{list.TTM}}</span>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div class="component-box component-box--align-right">
                            <a href="financial_link/exchange.html" class="btn btn--link"><span class="btn__txt">+ 더보기</span></a>
                        </div>
                    </div>
                    <div class="component-area component-area-tax" v-bind:class="{'tab-cnt--active' : taxSchedule}">
                        <div class="component-group component-group__table">
                            <div class="table">
                                <table>
                                    <caption><span class="blind">월별 세무 일정</span></caption>
                                    <colgroup>
                                        <col style="width: 70px">
                                        <col>
                                        <col style="width: 192px">
                                    </colgroup>
                                    <thead>
                                        <tr>
                                            <th scope="col" class="table__th">월/일</th>
                                            <th scope="col" class="table__th">일정</th>
                                            <th scope="col" class="table__th">비고</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="list in taxPlanList">
                                            <td class="table__td">
                                                <span class="table__txt">{{list.taxDate}}</span>
                                            </td>
                                            <td class="table__td table__title">
                                                <span class="table__txt">{{list.taxPlan}}</span>
                                            </td>
                                            <td class="table__td">
                                                <span class="table__txt table__txt--gray">{{list.note}}</span>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div class="component-box component-box--align-right">
                            <a href="financial_link/tax_schedule.html" class="btn btn--link"><span class="btn__txt">+ 더보기</span></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="grid-column grid-column3">
            <div class="section section--border">
                <div class="component-area component-area-user">
                    <div class="component-group align-center">
                        <div class="component-box"><!-- header__util-menu header__util-user -->
                            <span class="sp icon-user">
                                <img src="/resources/fim/img/icon-user-dummy.jpg" alt="유저 프로필 사진">
                            </span>
                        </div>                                
                        <div class="component-box align-left">
                            <span class="content-title content-title--bold">홍길동A 대리</span><br>
                            <span class="component-box__txt">사원번호 H20521</span>
                        </div>
                    </div>
                    <div class="component-group component-group-approvalbox">
                        <div class="component-box component-box--align-left align-center">
                            <a href="portal_management/approval_pending.html">
                                <span class="content-title">미결함</span>
                                <span class="component-box__txt component-box__txt--bold txt--orange">6</span>
                            </a>
                        </div>
                        <div class="component-box component-box--align-left align-center">
                            <a href="portal_management/approval_pre.html">
                                <span class="content-title">예정함</span>
                                <span class="component-box__txt component-box__txt--bold txt--blue">1</span>
                            </a>
                        </div>
                        <div class="component-box component-box--align-left align-center">
                            <a href="portal_management/approval_progress.html">
                                <span class="content-title">진행함</span>
                                <span class="component-box__txt component-box__txt--bold">21</span>
                            </a>
                        </div>
                    </div>
                </div>
                <div class="component-area component-area-bookmark">
                    <div class="section__title">즐겨찾는 메뉴</div>
                    <div class="component-group">
                        <div class="table table--scroll-vertical">
                            <div class="table-box">
                                <table>
                                    <caption>나의 즐겨찾는 메뉴 리스트</caption>   
                                    <tbody>
                                        <tr>
                                            <td class="table__td">
                                                <a href="company_card/account/account_info.html" class="table__title">
                                                   <span class="table__txt"><span class="sp icon-bookmark--small"></span>법인카드 정산</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="expense_management/workshop.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>워크샵 정산</span>
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/eis/current_pjt.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>프로젝트 현황</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="expense_management/cafeteria_mobile.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>모바일식권 정산</span>
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/work/work_history.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>작업내역 관리</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="business_management/car/car_drive.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>차량 운행 일지</span>
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/car/car_cost_system.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>업무용 승용차 명세서 확인</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="portal_management/exchange.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>환율 정보</span>
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="expense_management/bill/tax_bill.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>전자세금계산서</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="business_management/eis/accumulation.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>누적손익</span>
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/budget/detail_result.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>실적 상세 내역</span>
                                                </a>
                                            </td>
                                            <td class="table__td">
                                                <a href="company_card/managing/limit_management.html" class="table__title">
                                                    <span class="table__txt"><span class="sp icon-bookmark--small"></span>카드 한도 관리</span>
                                                </a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                    <a href="#" class="btn btn--link">
                        <span class="sp icon-modify"></span>
                    </a>
                </div>
                <div class="component-area component-area-recent">
                    <div class="section__title">최근 이용 내역</div>
                    <div class="component-group">
                        <div class="table table--scroll-vertical">
                            <div class="table-box">
                                <table>
                                    <caption>최근 이용한 메뉴 리스트</caption>
                                    <tbody>
                                        <tr>
                                            <td class="table__td">
                                                <a href="company_card/account/account_info.html"><span class="table__txt">법인카드 정산</span></a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/work/user_ratio.html"><span class="table__txt">나의 투입률 관리</span></a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="expense_management/bill/tax_bill.html"><span class="table__txt">전자세금계산서</span></a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/car/car_cost_system.html"><span class="table__txt">업무용 승용차 명세서 확인</span></a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="expense_management/approval/expense.html"><span class="table__txt">지출결의서</span></a>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td class="table__td">
                                                <a href="business_management/budget/detail_result.html"><span class="table__txt">실적 상세 내역</span></a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>  
</div>

<div class="popup-wrap popup-dialog--simple popup-dialog--bookmark">
    <div class="popup-wrap__align-box">
        <div class="popup-wrap__inner">
            <div class="popup__inner-wrap">
                <div class="popup__header">
                    <strong class="popup__header-txt">즐겨찾는 메뉴</strong>
                    <a href="#" class="btn btn--link">
                        <span class="sp icon-modify"></span>
                    </a>
                </div>
                <div class="popup__contents">
                    <ul class="popup__lst">
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">법인카드 정산</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">나의 투입률 관리</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">당월손익</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">전자세금계산서</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">택시비 정산</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">환율 정보</span>
                            </a>
                        </li>
                    </ul>

                </div>
            </div>
            <div class="popup__inner-wrap">
                <div class="popup__header">
                    <strong class="popup__header-txt">최근 이용 내역</strong>
                </div>
                <div class="popup__contents">
                    <ul class="popup__lst">
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">법인카드 정산</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">차량운행일지</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">공지사항</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">스마일락(동호회) 정산</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">부서/프로젝트 관리</span>
                            </a>
                        </li>
                        <li class="popup__lst-item">
                            <a href="#" class="popup__lst-menu">
                                <span class="popup__menu-txt">환율 정보</span>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
            <button type="button" class="popup__btn-close">
                <span class="sp icon-close"><span class="blind">닫기</span></span>
            </button>
        </div>
    </div>
</div>
